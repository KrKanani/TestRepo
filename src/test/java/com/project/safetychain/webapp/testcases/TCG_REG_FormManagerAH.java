...
public class TCG_REG_FormManagerAH extends TestBase {
	String av = null;
	...
	// Data
	public static HashMap<String, List<String>> LocationMap;
	public static HashMap<String, List<String>> resourceCatMap;
	public static String verificationName;
	public static String verificationName38630;
	public static String rolename = "SuperAdmin";
	public static String numericFieldNm = "Numeric Field", numericFieldValue = "20" , freeTextFieldNm = "Free Text Field";
	public static String formName1;
	public static String commonformsName;
	...
	public static com.project.safetychain.api.models.support.Support.FormFieldParams numericField = null;
	public static com.project.safetychain.api.models.support.Support.FormFieldParams submitNumericField = null;

	public static String roleDetails [][] = {{Verifications.SUPERADMIN_ROLE_NAME,Verifications.SUPERADMIN_ROLE_ID}};
	...
	public void groupInit() throws InterruptedException {
		...
		locationCategoryValue = "Loc_Cat_18.11_16.49.49";
		locationInstanceValue = "Location_Inst_18.11_16.49.49";
		customersCategoryValue = "Cust_Cat_18.11_16.49.49";
		customersInstanceValue = "Cust_Inst_18.11_16.49.49";
		commonformsName = "Auto_ChkForm_FM_PD_19.11_16.19.26";
//		commonformsName = CommonMethods.dynamicText("FM_Common");
...
//		customersInstanceValue = CommonMethods.dynamicText("Cust_Inst");
		...
		verificationComment1 = "AutoComment1_18.11_16.49.49";
//		verificationComment1 = CommonMethods.dynamicText("AutoComment1");
		...
		verificationName = "AutoVN_18.11_16.49.49";
//		verificationName = CommonMethods.dynamicText("AutoVN");
		verificationName38630 = CommonMethods.dynamicText("AutoVN38630");
		...
		checkFormNamefmFormViewComment = CommonMethods.dynamicText("Auto_FM_Chk_fmFormViewCommt");
		...
		numericField = new com.project.safetychain.api.models.support.Support.FormFieldParams();
		numericField.DPTFieldType = DPT_FIELD_TYPES.NUMERIC;
		numericField.Name = numericFieldNm;
		
		// API - Add values to fields in form
		submitNumericField = new com.project.safetychain.api.models.support.Support.FormFieldParams();
		submitNumericField.Name = numericFieldNm;
		submitNumericField.Value = numericFieldValue;
...
//			TestValidation.IsTrue(!shortNames.isEmpty(), 
//					"Able to create and release form:" + checkFormNamefm4,
//					"Could NOT able to create and release forms:" +checkFormNamefm4);
//
//			// ------------------------------------------------------------------------------------------------
//			// API - Verification creation
//			
//			
//
//			VerificationDetails verificationDetails = new VerificationDetails();
//			verificationDetails.VerificationName = verificationName;
//			verificationDetails.IsEnable = true;
//			verificationDetails.IsCommentsEnabled = true;
//			verificationDetails.comments = Arrays.asList(verificationComment1);
//			verificationDetails.roles = roleDetails;
//			SimpleDataResponse createVerification = verifications.createVerification(verificationDetails);
//			if (!createVerification.Status) {
...
//			}
	...
	public void TestCase_36144() throws Exception {
		...
		TestValidation.IsTrue(clickFormDesigner, 
				"Able to click Form Designer button",
				"Failed to navigate to Form Designer button");

		fm = hp.clickFormsManagerMenu();
		boolean clickTaskSchdulerBtn = fm.clickTaskSchdulerBtn();
		TestValidation.IsTrue(clickTaskSchdulerBtn, 
				"Able to click Task Scheduler button",
	...
	public void TestCase_38633() throws Exception {
		...
		try {
			...
			TestValidation.IsTrue(verification, 
					"Able to click on Verification", 
					"Unable to click on Verification");

			boolean setPageTo40 = controlActions.selectDropdown(PGN_SIZE.SIZE_40, fm.Verification_PaginationDrd);
			hp.Sync();
			TestValidation.IsTrue(setPageTo40, 
					"Able to set on Pagination to 40", 
					"Unable to set on Pagination to 40");

			boolean setPageTo50 = controlActions.selectDropdown(PGN_SIZE.SIZE_50, fm.Verification_PaginationDrd);
			hp.Sync();
			TestValidation.IsTrue(setPageTo50, 
					"Able to set on Pagination to 50", 
					"Unable to set on Pagination to 50");
		} finally {
	...
	public void TestCase_38635() throws Exception {
		...
		try {
			...
			TestValidation.IsTrue(location, 
					"Able to click on Location", 
	...
	@Test(description = "Forms Manager | Verification of search bar functionality")
	public void TestCase_38636() throws Exception {
		...
		try {
			...
			TestValidation.IsTrue(location, 
					"Able to search and highlight on Location",
	...
	public void TestCase_38634() throws Exception {
		...
		try {
			...
			TestValidation.IsTrue(selectallforms, 
					"Able to select location forms",
					"Unable to search and highlight on Location");

			boolean clickOnNextPage = fm.clickOnNextPageArrow();
			TestValidation.IsTrue(clickOnNextPage, 
					"Able to click on next Page with Arrow",
					"Unable to click on next page ");

			boolean selectallforms1 = fm.linkingVerificationWithAllFormsNextPageArrow(verificationName);
			TestValidation.IsTrue(selectallforms1, 
					"Able to select location forms",
					"Unable to search and highlight on Location");
	...
	public void TestCase_38629() throws Exception {
		try {
			...
			LinkedHashMap<String, String> shortNames = formCreationWrapper.API_Wrapper_Forms(fp);
			TestValidation.IsTrue(!shortNames.isEmpty(), 
					"Able to create and release form:" + checkFormFM,
			...
			TestValidation.IsTrue(formLinkedwithVerifications, 
					  "LINKED verification " + verificationName + " to form " + checkFormFM, 
					  "Failed to link verification " + verificationName + " to form " + checkFormFM);
			...
			boolean applyFilterAndOpenForm = fbp.applyFilterAndOpenForDetails(COLUMNHEADER.FORMNAME, checkFormFM);
			...
			TestValidation.IsTrue(submitData, 
					"Able to submit the form with data:" + checkFormFM,
			...
			TestValidation.IsTrue(openRecord, 
					"OPENED record for form - " + checkFormFM,
			...
			boolean openHistPopup = frp.clickHistoryIcon();
			TestValidation.IsTrue(openHistPopup, "OPENED History popup for record - " + checkFormFM,
					"Failed to History popup for record - " + checkFormFM);
			
			String currentDate = dt.getTodayDate("MM/dd/YYYY", timezoneCode);
	...
	public void TestCase_38647() throws Exception {
		try {
			...
			TestValidation.IsTrue(searchingForm, 
					"OPENED the search form - " + checkFormNamefm1,
			...
			fbp = hp.clickFSQABrowserMenu();
			boolean navigate = fbp.selectResourceDropDownandNavigate(FSQARESOURCE.CUSTOMERS, FSQATAB.FORMS);
			TestValidation.IsTrue(navigate, 
					"Navigated to FSQABrowser>FormsTab",
					"Failed to navigate to FSQABrowser>Forms Tab");

			boolean applyFilterAndOpenForDetails = fbp.applyFilterAndOpenForDetails(COLUMNHEADER.FORMNAME, checkFormNamefm1);
			TestValidation.IsFalse(applyFilterAndOpenForDetails, 
	...
	public void TestCase_38648() throws Exception {
		try {
			...
			LinkedHashMap<String, String> shortNames = formCreationWrapper.API_Wrapper_Forms(fp);
			TestValidation.IsTrue(!shortNames.isEmpty(), 
					"Able to create and release form:" + checkFormNamefm2,
			...
			TestValidation.IsTrue(searchingForm, 
					"OPENED the search form - " + checkFormNamefm2,
			...
			boolean applyFilterAndOpenForDetails = fbp.applyFilterAndOpenForDetails(COLUMNHEADER.FORMNAME, checkFormNamefm2);
			TestValidation.IsFalse(applyFilterAndOpenForDetails, 
					"Verify form is displayed in Forms Tab",
					"Form is NOT displayed in Forms Tab");

			fm = hp.clickFormsManagerMenu();
			boolean searchForm = fm.searchForm(checkFormNamefm2);
			...
			boolean applyFilterAndOpenForm = fbp.applyFilterAndOpenForDetails(COLUMNHEADER.FORMNAME, checkFormNamefm2);
			TestValidation.IsTrue(applyFilterAndOpenForm, 
					"Verify form IS DISPLAYED in Forms Tab",
					"Form is NOT displayed in Forms Tab");
	...
	public void TestCase_38630() throws Exception {
		...
		try {
			...
			LinkedHashMap<String, String> shortNames = formCreationWrapper.API_Wrapper_Forms(fp);
			TestValidation.IsTrue(!shortNames.isEmpty(), 
					"Able to create and release form:" + checkFormNamefm5,
			...
			fpSubmit.FormName = checkFormNamefm5;
			...
			// link verification with form
			fm = hp.clickFormsManagerMenu();
			boolean formLinkedwithVerifications = fm.linkVerificationwithForm(verificationName, checkFormNamefm5);
			TestValidation.IsTrue(formLinkedwithVerifications, 
					  "LINKED verification " + verificationName + " to form " + checkFormNamefm5, 
					  "Failed to link verification " + verificationName + " to form " + checkFormNamefm5);
			...
			boolean openRecord1 = fbp.applyFilterAndOpenForDetails(COLUMNHEADER.FORMNAME, checkFormNamefm5);
			TestValidation.IsTrue(openRecord1, 
					"OPENED record for form - " + checkFormNamefm5,
					"Failed to open record for form - " + checkFormNamefm5);

			boolean signOffRecord = frp.signoffRecordWithVerfication(verificationName, verificationComment1);
			TestValidation.IsTrue(signOffRecord, 
					"SIGNED record for form - " + checkFormNamefm5,
			...
			// Calling function for verificationName delinking
			fm = hp.clickFormsManagerMenu();
			boolean delinkVerificationtoForm = fm.delinkVerificationwithForm(verificationName, checkFormNamefm5);
			TestValidation.IsTrue(delinkVerificationtoForm,
					"Verification " + verificationName + " is DELINKD with Form",
					"Failed to Delink the Verification " + verificationName + " with Form");
			...
			boolean applyFilterAndOpenForm = fbp.applyFilterAndOpenForDetails(COLUMNHEADER.FORMNAME, checkFormNamefm5);
			TestValidation.IsTrue(applyFilterAndOpenForm, 
					"Verify form is displayed in Forms Tab",
					"Form is NOT displayed in Forms Tab");
			...
			boolean submitFormWithDirectObservation = fbp.clickDirectObservationVerification(adminUsername,
					adminverificationPinTxt);
			TestValidation.IsFalse(submitFormWithDirectObservation,
					"Verify form is NOT saved with Direct Observation in Forms Tab",
					"Failed to verify whether Form is or is not saved with Direct Observation in Forms Tab");

			boolean submitData = fbForms.submitData(false, false, false, true, false);
			TestValidation.IsTrue(submitData, 
					"Able to submit the form with data:" + checkFormNamefm5,
			...
			boolean openRecord2 = fbp.applyFilterAndOpenForDetails(COLUMNHEADER.FORMNAME, checkFormNamefm5);
			TestValidation.IsTrue(openRecord2, 
					"OPENED record for form - " + checkFormNamefm5,
					"Failed to open record for form - " + checkFormNamefm5);

			boolean setVerification = frp.setVerficationForSignOff(verificationName);
			TestValidation.IsFalse(setVerification, 
					"Verification " + verificationName + " is NOT displayed hence cannot be set",
					"Failed to verify whether Verification " + verificationName + " is or is not displayed");
	...
	@Test(description = "Forms Manager | Verification of Details tab")
	public void TestCase_38644() throws Exception {
		try {

			fm = hp.clickFormsManagerMenu();
			boolean searchingForm = fm.searchAndFormSelection(commonformsName);
			TestValidation.IsTrue(searchingForm, 
					"OPENED the search form - " + commonformsName,
					"Failed to open searched form - " + commonformsName);
			
			boolean navigateToVerification = fm.clickVerificationTabViaOverview();
			TestValidation.IsTrue(navigateToVerification, 
					"NAVIGATED to Overview > Verifications tab",
					"Failed to Navigate to Overview > Verifications tab");

			WebElement FmOverviewVerificationName = controlActions.perform_GetElementByXPath(
					FormsManagerPageLocators.FM_OVERVIEW_VERIFICATION_NAME, "VERIFICATION_NAME", verificationName);
			TestValidation.IsTrue(FmOverviewVerificationName!=null, 
					"VERFIED that the verification " + verificationName + " is displayed",
					"Failed to Verify whether the verification " + verificationName + " is or is not displayed");
	...
	public void TestCase_38645() throws Exception {
		try {
			...
			boolean applyFilter = fbp.openAndApplySettingsForColumn(COLUMNHEADER.FORMNAME, 
					COLUMNSETTING.FILTER, commonformsName);
			TestValidation.IsTrue(applyFilter, 
					"Applied Filter for Form " + commonformsName, 
					"Failed to Apply filter for Form " + commonformsName);
			
			boolean editForm = fbp.editSelectForm(commonformsName);
			...
			boolean setFreeText = fdp.setTextBoxValue(FIELD_TYPES.FREE_TEXT, freeTextFieldNm);		
			TestValidation.IsTrue(setFreeText, 
					freeTextFieldNm + " ADDED successfully", 
					"Failed to Add " + freeTextFieldNm);
			...
			boolean releaseForm2 = fdp.releaseForm(commonformsName);
			TestValidation.IsTrue(releaseForm2, 
							       "SHOULD release form - " + commonformsName, 
								   "Failed since uncertain that the form " + commonformsName + " did/did not release'");
			...
			ArrayList<String> versions = controlActions.perform_convertListOfWebElements_ToListOfStrings(fm.FmOverviewVersionLst);
			boolean verifyVersion1 = versions.contains("1.0");
			boolean verifyVersion2 = versions.contains("2.0");
			TestValidation.IsTrue(verifyVersion1 && verifyVersion2, 
					"VERFIED both the versions of form - " + commonformsName,
					"Failed to open searched form - " + commonformsName);

			boolean clickOnFormVersion = fm.clickOnFormVersion(commonformsName);
			TestValidation.IsTrue(clickOnFormVersion, 
					"OPENED the search form - " + commonformsName,
					"Failed to open searched form - " + commonformsName);
	...
	public void TestCase_38646() throws Exception {
		...
		try {
			...
			fields.put(FIELD_TYPES.NUMERIC, Arrays.asList(numericFieldNm));
			...
			fp.FormName = checkFormNamefmFormViewComment;
			...
			TestValidation.IsTrue(releaseForm,
								  "RELEASED form - " + checkFormNamefmFormViewComment,
								  "Could NOT release form - " + checkFormNamefmFormViewComment);

			fm = hp.clickFormsManagerMenu();
			boolean searchingForm = fm.searchAndFormSelection(checkFormNamefmFormViewComment);
			TestValidation.IsTrue(searchingForm, 
					"OPENED the search form - " + checkFormNamefmFormViewComment,
			...
			boolean releaseMsg = fm.FmOverviewVersionCmmntContainer.getText().contains(fdp.automationReleaseFormMsg);
			boolean verifyVersion = fm.FmOverviewVersionCmmntContainer.getText().contains("1.0");
			TestValidation.IsTrue(releaseMsg && verifyVersion, 
					"VERIFIED release comment for the version 1.0 of the form - " + checkFormNamefmFormViewComment,
					"Failed to Verify release comment for the version 1.0 of the form - " + checkFormNamefmFormViewComment);
			
			boolean clickOnCloseButton = fm.clickOverviewFormClose();
			TestValidation.IsTrue(clickOnCloseButton, 
					"Able to Close the page", 
					"Failed to Close the page ");
	...
	public void TestCase_38654() throws Exception {
		try {
			...
			WebElement LocationSelectionTxt = controlActions.perform_GetElementByXPath(
					FormsManagerPageLocators.LOCATIONSELECTION_TXT, "LOCATION", locationInstanceValue);
			TestValidation.IsTrue(LocationSelectionTxt!=null, 
					"VERFIED that the location " + LocationSelectionTxt + " is displayed",
					"Failed to Verify whether the location " + LocationSelectionTxt + " is or is not displayed");
			
			int locationCount = fm.LocationTabLocNameLst.size();
			TestValidation.IsTrue(locationCount >= 1, 
					"VERFIED that locations are displayed",
					"Failed to Verify whether the locations are displayed or not");
