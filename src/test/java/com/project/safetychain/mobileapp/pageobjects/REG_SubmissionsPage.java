...
public class REG_SubmissionsPage extends TestBase {
	...
	public REG_SubmissionsPage(AppiumDriver<MobileElement> driver) {
	...
	@FindBy(id = SavedPageLocators.FORM_NAME_LIST)
	public List<WebElement> formGrid;
	...
	public boolean SearchResource(String resourceNm) {
		try {
			...
			ControlActions_MobileApp.WaitforelementToBeClickable(resourceSearchField);
	...
	public boolean clickResubmitAll() {
		try {
			if (ControlActions_MobileApp.isElementDisplayed(resubmitBtn)) {
				...
				ControlActions_MobileApp.click(resubmitPopupBtn);
				try {
					ControlActions_MobileApp.waitForVisibility(resubmitPopupBtn, 30);
					ControlActions_MobileApp.click(resubmitPopupBtn);
					try {
						ControlActions_MobileApp.waitForVisibility(resubmitPopupBtn, 30);
						ControlActions_MobileApp.click(resubmitPopupBtn);
	...
	public boolean verifyFieldValues(String fieldName, String fieldValue) {
		...
		try {
			...
			case "AGG1-SUM":
			case "AGG1-SumRange":
			case "NegativeCountRange":
			case "PositiveCountRange":
			case "CountRangeNP":
			case "NegativeSumRange":
			case "PositiveSumRange":
			case "SumRangeNP":
			case "Sum Range":
			case "Count Range":
			case "Average":
	...
	public boolean VerifyFieldValuesUsingScroll(String expectedVal, String FieldValue)
		...
		try {
			...
			do {
				try {
					Field = appiumDriver.findElement(By.xpath(FieldValue));
					String isEnabled = Field.getAttribute("enabled");
					@SuppressWarnings("unused")
					String isClickable = Field.getAttribute("clickable");

					String ActualFieldValue = Field.getText();
					logInfo("Actual value =" + ActualFieldValue);

					if (isEnabled.equalsIgnoreCase("false")) {
						logInfo(ActualFieldValue + " Field is disabled");
					} else {
						isPresent = false;
					}
	...
	public boolean EnterFieldValuesUsingSendKeysScrolling(String FieldValue, String FieldName)
			throws InterruptedException, ElementNotVisibleException {
		try {
			...
			do {
				try {
					Field = appiumDriver.findElement(By.xpath(fieldValue));
					ControlActions_MobileApp.waitForVisibility(Field, 10);
					ControlActions_MobileApp.sendKeys(Field, FieldValue);
					ControlActions_MobileApp.performTabAction();
					logInfo("Field Value has Entered");
					isPresent = true;
					logInfo("Field Value has Entered");
	...
	public boolean SelectFieldValuesUsingScrolling(String FieldValue, String FieldName)
		...
		try {
			...
			do {
				try {
					Field = appiumDriver.findElement(By.xpath(fieldValue));
					Point value = Field.getLocation();
					int x = value.x;
					int y = value.y;
					int y1 = value.y + 100;
					logInfo("x = " + x + " y = " + y + " y1 =" + y1);
					ControlActions_MobileApp.click(Field);
					isPresent = true;
					logInfo("Field Value has Selected");
	...
	public boolean QuestionareForm_fillFormDetailsWithAttachment()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			UploadImage oUploadImage = new UploadImage(appiumDriver);

			Boolean txtNumeric = EnterFieldValuesUsingScrolling("10", "Numeric");
			...
			String date1 = ControlActions_MobileApp.AddDaystoToddaysDate(0);
	...
	public boolean fillFormDetailsWithAttachmentForAllFields()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			Boolean txtNumeric = EnterFieldValuesUsingScrolling("-100", "Numeric");
			TestValidation.IsTrue(txtNumeric, "Numeric Field value entered successfully",
					"Failed to enter field value");

			ControlActions_MobileApp.attachImageThroughCamera();

			Boolean Identifier1 = SelectFieldValuesUsingScrolling("7", "Identifier1");
			TestValidation.IsTrue(Identifier1, "SelectOne Field value Selected successfully",
					"Failed to enter field value");
			...
			TestValidation.IsTrue(txtSelectOne1, "SelectOne Field value Selected successfully",
					"Failed to enter field value");
			ControlActions_MobileApp.attachImageThroughCamera();
			// oUploadImage.verifySelectedImageThroughCamera("C");
			...
			ControlActions_MobileApp.attachImageThroughGallery();
	...
	public boolean fillFormDetailsWithAttachmentForAllFields2()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			UploadImage oUploadImage = new UploadImage(appiumDriver);

			Boolean txtNumeric = EnterFieldValuesUsingScrolling("-99.10", "Numeric2");
			TestValidation.IsTrue(txtNumeric, "Numeric Field value entered successfully",
					"Failed to enter field value");

			ControlActions_MobileApp.attachImageThroughCamera();

			Boolean txtIdentifier = EnterFieldValuesUsingSendKeysScrolling("77777", "IDNFieldGroup");
			TestValidation.IsTrue(txtIdentifier, "Identifier Field value entered successfully",
					"Failed to enter field value");

			Boolean txtSelectOne1 = SelectFieldValuesUsingScrolling("C", "SelectOne2");
			TestValidation.IsTrue(txtSelectOne1, "SelectOne Field value Selected successfully",
					"Failed to enter field value");
			ControlActions_MobileApp.attachImageThroughCamera();
			oUploadImage.verifySelectedImageThroughCamera("C");

			Boolean txtMultiSelectOne1 = SelectFieldValuesUsingScrolling("A", "MultiSelect2");
			TestValidation.IsTrue(txtMultiSelectOne1, "MultiSelect Field value Selected successfully",
					"Failed to enter field value");
			ControlActions_MobileApp.attachImageThroughCamera();

			Boolean txtParagraph = EnterFieldValuesUsingScrolling(
					"This is Paragraph Text and has to be checked with Maxixmun Field Values!@#$%^&*()", "ParaGraph2");
			TestValidation.IsTrue(txtParagraph, "Paragraph Field value entered successfully",
					"Failed to enter field value");
			ControlActions_MobileApp.attachImageThroughCamera();
			Boolean txtSingleLineText = EnterFieldValuesUsingScrolling(
					"Single Line Text Value Has Entered Successsfully", "SingleLineText2");
			TestValidation.IsTrue(txtSingleLineText, "SingleLineText Field value entered successfully",
					"Failed to enter field value");
			...
			Boolean txtDate1 = SelectDateFieldValuesUsingScrolling("Date2", date);
			TestValidation.IsTrue(txtDate1, "Date Field value entered successfully", "Failed to enter field value");
			String dateSelected = ControlActions_MobileApp.AddDaystoToddaysDateInGivenFormat("M/d/YYYY", 0);
			logInfo("Date Field Value = " + dateSelected);
			ControlActions_MobileApp.attachImageThroughGallery();
			...
			Boolean txtDateTime1 = SelectDateFieldValuesUsingScrolling("DateTime2", date1);
			TestValidation.IsTrue(txtDateTime1, "Date Time Field value entered successfully",
					"Failed to enter field value");
			ControlActions_MobileApp.attachImageThroughGallery();
			...
			Boolean txtTime1 = SelectTimeFieldValuesUsingScrolling("Time2");
			TestValidation.IsTrue(txtTime1, "Time Field value entered successfully", "Failed to enter field value");
			ControlActions_MobileApp.attachImageThroughGallery();
			return true;
		} catch (Exception ex) {
	...
	public boolean fillFormDetailsWithoutAttachmentForAllFields()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			Boolean txtNumeric = EnterFieldValuesUsingScrolling("-100", "Numeric2");
			TestValidation.IsTrue(txtNumeric, "Numeric Field value entered successfully",
					"Failed to enter field value");

			Boolean Identifier1 = EnterFieldValuesUsingSendKeysScrolling("78966", "Identifier2");
			TestValidation.IsTrue(Identifier1, "SelectOne Field value Selected successfully",
					"Failed to enter field value");
	...
	public boolean checkForm_fillFormDetailsWithoutAttachment()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			Boolean txtNumeric = EnterFieldValuesUsingScrolling("10", "Numeric");
			TestValidation.IsTrue(txtNumeric, "Numeric Field value entered successfully",
			...
			Boolean txtDate2 = SelectDateFieldValuesUsingScrolling("Date2", date1);
			TestValidation.IsTrue(txtDate2, "Date Field value entered successfully", "Failed to enter field value");
			String dateSelected1 = ControlActions_MobileApp.AddDaystoToddaysDateInGivenFormat("M/d/YY", 1);
			logInfo("Date Field Value = " + dateSelected1);

			Boolean Identifier1 = SelectFieldValuesUsingScrolling("1", "Identifier1");
			TestValidation.IsTrue(Identifier1, "SelectOne Field value Selected successfully",
					"Failed to enter field value");

			Boolean txtSelectOne1 = SelectFieldValuesUsingScrolling("B", "SelectOne");
			TestValidation.IsTrue(txtSelectOne1, "SelectOne Field value Selected successfully",
					"Failed to enter field value");

			Boolean txtIdnFieldGroup1 = SelectFieldValuesUsingScrolling("TEST 1", "IDNFieldGroup2");
			TestValidation.IsTrue(txtIdnFieldGroup1, "Identifier1 Field value Selected successfully",
					"Failed to enter field value");
			...
			Boolean txtIdnNumeric = EnterFieldValuesUsingScrolling("100", "Numeric1");
			TestValidation.IsTrue(txtIdnNumeric, "Identifier Numeric Field value entered successfully",
					"Failed to enter field value");
			...
			Boolean txtNumeric3 = EnterFieldValuesUsingScrolling("-100", "Numeric3");
			TestValidation.IsTrue(txtNumeric3, "Numeric Field value entered successfully",
					"Failed to enter field value");
			ControlActions_MobileApp.swipe(400, 700, 400, 100);
			ControlActions_MobileApp.swipe(400, 700, 400, 100);

			Boolean txtNumeric4 = EnterFieldValuesForRepeatUsingScrolling("5000", "Numeric4");
			TestValidation.IsTrue(txtNumeric4, "Numeric Field value entered successfully",
					"Failed to enter field value");

			Boolean txtNumeric5 = EnterFieldValuesForRepeatUsingScrolling("8000", "Numeric5");
			TestValidation.IsTrue(txtNumeric5, "Numeric Field value entered successfully",
					"Failed to enter field value");
			...
			Boolean txtSingleLineText = EnterFieldValuesUsingScrolling(
					"Single Line Text Value Has Entered Successsfully", "FreeText1");
			TestValidation.IsTrue(txtSingleLineText, "SingleLineText Field value entered successfully",
					"Failed to enter field value");
	...
	public boolean checkForm_fillFormDetailsWithAttachment()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			UploadImage oUploadImage = new UploadImage(appiumDriver);
	...
	public boolean QuestionareForm_fillFormDetailsWithoutAttachment()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			Boolean txtNumeric = EnterFieldValuesUsingScrolling("10", "Numeric");
			TestValidation.IsTrue(txtNumeric, "Numeric Field value entered successfully",
	...
	public boolean auditForm_fillFormDetailsWithoutAttachment()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			Boolean txtParagraph = EnterFieldValuesUsingScrolling(
					"This is Paragraph Text and has to be checked with Maxixmun Field Values!@#$%^&*()", "ParaGraph1");
			...
			Boolean txtSelectOne1 = SelectFieldValuesUsingScrolling("2", "SelectOne1");
			TestValidation.IsTrue(txtSelectOne1, "SelectOne Field value Selected successfully",
					"Failed to enter field value");

			Boolean txtNumeric = EnterFieldValuesUsingScrolling("-10000", "Numeric1");
			TestValidation.IsTrue(txtNumeric, "Numeric Field value entered successfully",
					"Failed to enter field value");

			Boolean txtMultiSelectOne1 = SelectFieldValuesUsingScrolling("80", "MultiSelect");
			TestValidation.IsTrue(txtMultiSelectOne1, "MultiSelect Field value Selected successfully",
					"Failed to enter field value");

			Boolean Identifier1 = SelectFieldValuesUsingScrolling("G3", "IDNSection1");
			TestValidation.IsTrue(Identifier1, "SelectOne Field value Selected successfully",
					"Failed to enter field value");
			...
			Boolean txtIdentifier = EnterFieldValuesUsingSendKeysScrolling("77777", "IDNText2");
			TestValidation.IsTrue(txtIdentifier, "Identifier Field value entered successfully",
					"Failed to enter field value");

			Boolean txtIdentifier2 = EnterFieldValuesUsingScrolling("Identifiert@1234", "IDNText3");
			TestValidation.IsTrue(txtIdentifier2, "Identifier Field value entered successfully",
					"Failed to enter field value");

			String fieldValue = ControlActions_MobileApp
					.perform_GetElementByXPath2(REG_SubmissionPageLocators.INPUT_FIELD, "FIELDNAME", "IDNText2");

			appiumDriver.findElement(By.xpath(fieldValue)).click();
	...
	public boolean auditForm_fillFormDetailsWithAttachment()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			UploadImage oUploadImage = new UploadImage(appiumDriver);
			...
			oUploadImage.verifySelectedImageThroughCamera("80");
	...
	public boolean clickofflineToggleButton() {
		try {
			...
			ControlActions_MobileApp.click(offlineToggle);
			ControlActions_MobileApp.click(settingsBckBtn);
	...
	public boolean verifyStatusAfterclickingResubmitAllButton(String FormName, String Status) {
		try {
			homePage.ClickMenu(homePage.submissionsSubMenu);
			clickResubmitAll();
			ControlActions_MobileApp.ClearText(searchField);
			ControlActions_MobileApp.actionEnterText(searchField, FormName);
			ControlActions_MobileApp.click(sortBtn);
			boolean status = checkFormStatusFromSubmissionsSubMenu(FormName, Status);
	...
	public boolean VerifyfieldDetailsandAttachments()
			throws ElementNotVisibleException, InterruptedException, ParseException {
		try {
			UploadImage oUploadImage = new UploadImage(appiumDriver);
			...
			ControlActions_MobileApp.attachImageThroughCamera();
			oUploadImage.verifySelectedImageThroughCamera("-100");
	...
	public boolean checkFormListStatusFromSubmissionsMenu(String FormName) {
		Boolean isPresent = false;
		String fieldValue = null;
		List<MobileElement> formsList = null;

		try {

			fieldValue = ControlActions_MobileApp.perform_GetElementByXPath2(REG_SubmissionPageLocators.FORM_NAME_LIST,
					"FORMNAME", FormName);

			logInfo("Form Name = " + fieldValue);

			formsList = appiumDriver.findElements(By.xpath(fieldValue));

			int count = formsList.size();
			logInfo("Form Grid Count = " + count);
			if (count == 2) {
				for (int i = 0; i < count; i++) {

					String formStatus = formsList.get(i).getText();

					if (formStatus.equals("RECEIVED")) {
						isPresent = true;
						logInfo("Form Status has been verified");
					} else {
						isPresent = false;
						logInfo("Couldnt verified form Status");
					}
				}
			...
			logError("Failed to Verify form Status - " + e.getMessage());
			return isPresent;
		}
	}

	public boolean verifyNoSubmissionsMessage() {
		try {
			...
			if (IsPresent) {
				...
				if (FormText.equals("No Submissions")) {
					logInfo("\"No Submissions\" is Displayed");
					return true;
				} else {
					logInfo("\"No Submissions\" is NOT Displayed");
					return false;
				}
			}

			else {
				logInfo("Message Element is Not there to verify message");
				return false;
			}

		} catch (Exception ex) {
			logInfo("Could Not Verify \"No Submissions\" messge" + ex.getMessage());
			return false;
		}
	}
